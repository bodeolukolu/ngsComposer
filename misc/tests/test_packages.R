...
if (suppressMessages(!require(ggplot2, lib.loc = .libPaths()[1]))) {
  ...
  suppressMessages(install.packages('ggplot2', dependencies=TRUE, quiet=TRUE, repos='http://cran.us.r-project.org'))
  }
suppressMessages(library(ggplot2))
if (suppressMessages(!require(Cairo, lib.loc = .libPaths()[1]))) {
  print(paste('Installing dependency (Cairo) into', .libPaths()[1]))
  suppressMessages(install.packages('Cairo', dependencies=TRUE, quiet=TRUE, repos='http://cran.us.r-project.org'))
  }
